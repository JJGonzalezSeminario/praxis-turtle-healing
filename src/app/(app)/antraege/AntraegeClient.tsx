...
interface Props {
  ...
  isAdmin: boolean
  userId: string
  leaveBalance: { total: number; used: number }
}

export function AntraegeClient({ initialRequests, isAdmin, userId, leaveBalance: initialBalance }: Props) {
  ...
  const handleAction = async (
    ...
    // Optimistisches Update: Status sofort in der UI ändern
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-zinc-900 tracking-tight flex items-center gap-3">
            ...
            {isAdmin ? 'Team-Anträge verwalten' : 'Meine Anträge'}
          </h1>
          <p className="text-zinc-500 font-medium mt-2">
            {isAdmin ? 'Hier verwaltest du die Urlaubs- und Abwesenheitsanträge deines Teams.' : 'Urlaub, Schichttausch und Abwesenheiten verwalten.'}
          </p>
        </div>
      ...
      {isAdmin && (
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar">
          <button onClick={() => setAdminFilter('ausstehend')} className={cn("px-4 py-2.5 rounded-xl text-sm font-bold transition whitespace-nowrap", adminFilter === 'ausstehend' ? "bg-teal-600 text-white shadow-md" : "bg-white text-zinc-500 border border-zinc-200 hover:bg-zinc-50")}>
            Zu bearbeiten
          </button>
          <button onClick={() => setAdminFilter('historie')} className={cn("px-4 py-2.5 rounded-xl text-sm font-bold transition whitespace-nowrap", adminFilter === 'historie' ? "bg-teal-600 text-white shadow-md" : "bg-white text-zinc-500 border border-zinc-200 hover:bg-zinc-50")}>
            Historie (Erledigt)
          </button>
          <button onClick={() => setAdminFilter('alle')} className={cn("px-4 py-2.5 rounded-xl text-sm font-bold transition whitespace-nowrap", adminFilter === 'alle' ? "bg-teal-600 text-white shadow-md" : "bg-white text-zinc-500 border border-zinc-200 hover:bg-zinc-50")}>
            Alle anzeigen
          </button>
        </div>
      )}
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg border border-zinc-200 overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-1">
                ...
                <select className="w-full border-2 border-zinc-100 p-3.5 rounded-xl outline-none focus:border-teal-500 font-bold text-zinc-800 bg-white" value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })}>
