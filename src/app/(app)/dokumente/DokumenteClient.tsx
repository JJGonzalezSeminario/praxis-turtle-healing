...
export function DokumenteClient({ initialDocuments, isAdmin, userId }: Props) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: newEntry, error: dbError } = await supabase.from('documents').insert([{
        title: newDoc.title,
      ...
      if (newEntry) {
        setDocuments([newEntry[0], ...documents])
      }
  ...
  const handleDelete = async (doc: any) => {
    ...
    try {
      ...
      if (pathParts.length > 1) {
        await supabase.storage.from('praxis_documents').remove([pathParts[1]])
      }
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg border border-zinc-200 overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpload} className="p-6 space-y-5">
              ...
              <button
                type="submit"
                disabled={isUploading}
                className={cn("w-full text-white font-bold py-4 rounded-xl shadow-lg transition-all flex justify-center items-center gap-2", isUploading ? "bg-zinc-400 cursor-not-allowed" : "bg-teal-600 hover:bg-teal-700 active:scale-95 shadow-teal-600/20")}
              >
