...
import { Shield, Clock, FileSpreadsheet } from 'lucide-react'
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-zinc-500 font-medium mt-2">
            Aktivitäten der Administratoren und Sicherheitsereignisse einsehen.
          </p>
        </div>
      </div>

      {/* FILTERBAR (Ausgelagert in Client Component zur Behebung des Event-Handler-Fehlers) */}
      <AuditLogFilter initialQuery={query} initialAction={action} />
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-zinc-50 border-b border-zinc-100 text-xs uppercase tracking-wider text-zinc-500 font-bold">
              ...
              <th className="p-4">Betroffener Nutzer</th>
              ...
              filteredLogs.map(log => {
                ...
                return (
                  <tr key={log.id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    {/* Ereignis */}
                    <td className="p-4">
                      <span className="bg-amber-50 text-amber-700 px-3 py-1 rounded-lg text-xs font-bold border border-amber-100 inline-flex items-center gap-1.5">
                        <Shield size={12} />
                        {actionLabel}
                      </span>
                    </td>
                    ...
                    {/* Details */}
                    <td className="p-4 pr-6 text-xs text-zinc-500 max-w-xs truncate">
